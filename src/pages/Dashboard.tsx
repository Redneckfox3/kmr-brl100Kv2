...
import { dbService, Refrigerant, Registration, AnnualBalance } from '../firebase';
...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const bals = await dbService.getAnnualBalances(currentYear);
  ...
  // Calculate stats
  const totalStockKg = refrigerants.reduce((sum, r) => {
    const bal = balances.find(b => b.refrigerant_id === r.id);
    const stockVal = bal ? bal.end_weight_kg : r.current_stock_kg;
    return sum + stockVal;
  }, 0);
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
          ...
          <div className="space-y-6">
            {refrigerants.map((ref) => {
              const bal = balances.find(b => b.refrigerant_id === ref.id);
              const stockVal = bal ? bal.end_weight_kg : ref.current_stock_kg;
              // Calculate percentage of cylinder capacity (assume 120kg max for display bar)
              const percentage = Math.min(100, (stockVal / 120) * 100);
              return (
